...
export const PLATFORMS: PlatformDefinition[] = [
  { id: 'linux-x64', goos: 'linux', goarch: 'amd64', toolsPlatform: 'linux', toolsArch: 'x64' },
  { id: 'linux-arm64', goos: 'linux', goarch: 'arm64', toolsPlatform: 'linux', toolsArch: 'arm64' },
  { id: 'darwin-x64', goos: 'darwin', goarch: 'amd64', toolsPlatform: 'darwin', toolsArch: 'x64' },
  { id: 'darwin-arm64', goos: 'darwin', goarch: 'arm64', toolsPlatform: 'darwin', toolsArch: 'arm64' },
  { id: 'win32-x64', goos: 'windows', goarch: 'amd64', toolsPlatform: 'win32', toolsArch: 'x64' },
  { id: 'win32-arm64', goos: 'windows', goarch: 'arm64', toolsPlatform: 'win32', toolsArch: 'arm64' },
];

export const BUILD_PLATFORMS: BuildConfig[] = PLATFORMS.map(({ goos, goarch }) => ({ goos, goarch }));
...
export const CORE_NPM_PACKAGES = [
  ...
  ...PLATFORMS.map(({ id }) => `${npmScopePath}/${npmConfig.corePlatformPrefix}${id}`),
];

export const DEPS_NPM_PACKAGES = [
  `${npmScopePath}/${npmConfig.depsPackageName}`,
  ...PLATFORMS.map(({ id }) => `${npmScopePath}/${npmConfig.depsPlatformPrefix}${id}`),
];
