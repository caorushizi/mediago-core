...
// progressTracker 进度节流器（200ms + 0.5% 阈值）
...
// shouldUpdate 判断是否应该发送进度更新
// 策略：首次必发、时间节流（200ms）、最小差异阈值（0.5%）
